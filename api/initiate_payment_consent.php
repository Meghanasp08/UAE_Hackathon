...
/**
 * Payment Consent Initiation Script
 * 
 * This script initiates a multi-payment consent flow for auto-sweep functionality.
 * It follows the Open Finance UAE multi-payment subscription pattern.
 * 
 * Flow:
 * 1. Generate code verifier and challenge
 * 2. Prepare encrypted PII with creditor details
 * 3. Create JWT for PAR endpoint (multi-payment subscription)
 ...
 * 8. index.php detects payment consent type and processes accordingly
 */
...
try {
    ...
    error_log("Step 1: Preparing encrypted PII for payment consent");
    ...
    // Step 2: Prepare request object JWT for PAR endpoint
    error_log("Step 2: Preparing request object JWT for PAR endpoint");
    ...
    error_log("PAR request_uri obtained: " . $requestUri);
    ...
    error_log("Redirecting to authorization URL for payment consent");
    ...
    error_log("Payment consent initiation error: " . $e->getMessage());
    
    // Redirect back to credit-line.php with error
    $_SESSION['payment_consent_error'] = $e->getMessage();
    header('Location: ../credit-line.php?error=consent_failed');
    exit;
}
